...
class TestScoreUpdater(unittest.TestCase):
    ...
    def test_1_score_parser_extracts_all_scores(self):
        ...
        # Sample HTML with all scores
        html = """
        <html>
        <body>
            <span data-original-title="Overall Score" data-content="4.5"></span>
            <span data-original-title="Style Score" data-content="4.2"></span>
            <span data-original-title="Story Score" data-content="4.8"></span>
            <span data-original-title="Grammar Score" data-content="4.6"></span>
            <span data-original-title="Character Score" data-content="4.4"></span>
        </body>
        </html>
        """
        ...
        self.assertEqual(scores['overall_score'], 4.5)
        self.assertEqual(scores['style_score'], 4.2)
        self.assertEqual(scores['story_score'], 4.8)
        self.assertEqual(scores['grammar_score'], 4.6)
        self.assertEqual(scores['character_score'], 4.4)
        print("ScoreParser correctly extracted all scores")
    
    def test_2_score_parser_handles_missing_scores(self):
        """Test that ScoreParser returns -1.0 for missing scores."""
        ...
        # All scores should default to -1.0
        for key in ['overall_score', 'style_score', 'story_score', 'grammar_score', 'character_score']:
            self.assertEqual(scores[key], -1.0, f"{key} should be -1.0 when missing")
        
        print("ScoreParser correctly defaults missing scores to -1.0")
    ...
    def test_4_score_scraper_extracts_scores(self):
        ...
        scores = scraper.scrape("https://www.royalroad.com/fiction/123/test")
        ...
        print("ScoreScraper correctly extracts scores")
    ...
    def test_5_update_scores_only_method_updates_only_scores(self):
        ...
        # Insert a test novel with known values
        test_data = {
        ...
        self.assertEqual(scores[1], 4.2)
        self.assertEqual(scores[2], 4.8)
        self.assertEqual(scores[3], 4.6)
        self.assertEqual(scores[4], 4.4)
    ...
    def test_6_update_scores_only_handles_invalid_values(self):
        ...
        invalid_scores = {
            ...
            'story_score': 'invalid',
            'grammar_score': 4.5,
            'character_score': -1  # Valid but negative
        }
        
        self.db.update_scores_only(novel_id, invalid_scores)
        
        # Verify invalid values become -1.0, valid ones are preserved
        self.db.cursor.execute("""
            SELECT overall_score, style_score, story_score, grammar_score, character_score
            FROM novels WHERE novel_id = ?
        """, (novel_id,))
        scores = self.db.cursor.fetchone()
        self.assertEqual(scores[0], -1.0)  # None -> -1.0
        self.assertEqual(scores[1], -1.0)  # '' -> -1.0
        self.assertEqual(scores[2], -1.0)  # 'invalid' -> -1.0
        self.assertEqual(scores[3], 4.5)   # Valid float preserved
        self.assertEqual(scores[4], -1.0)  # -1 is valid, but let's check it becomes -1.0 as float
    ...
    def test_7_score_crawler_integration(self):
        ...
        def mock_scrape(url):
            if 'novel1' in url:
                return {'overall_score': 4.0, 'style_score': 4.1, 'story_score': 4.2, 'grammar_score': 4.3, 'character_score': 4.4}
            elif 'novel2' in url:
                return {'overall_score': 3.5, 'style_score': 3.6, 'story_score': 3.7, 'grammar_score': 3.8, 'character_score': 3.9}
            elif 'novel3' in url:
                raise NovelDeleted()
            return {}
        ...
        # Novel 1
        self.assertEqual(results[0][1], 4.0)  # overall_score
        self.assertEqual(results[0][2], 4.1)  # style_score
        
        # Novel 2
        self.assertEqual(results[1][1], 3.5)
        self.assertEqual(results[1][2], 3.6)
    ...
    def test_9_concurrent_score_updates(self):
        ...
        # Create mock scraper returning different scores
        mock_scraper = Mock(spec=ScoreScraper)
        ...
        def mock_scrape(url):
            ...
            num = int(url[-1])
            return {
                'overall_score': num * 1.0,
                'style_score': num * 1.1,
                'story_score': num * 1.2,
                'grammar_score': num * 1.3,
                'character_score': num * 1.4
            }
        ...
        for i, row in enumerate(results):
            expected_overall = i * 1.0
            expected_style = i * 1.1
            self.assertAlmostEqual(row[1], expected_overall, places=1)
            self.assertAlmostEqual(row[2], expected_style, places=1)
